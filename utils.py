...
def organic_move_to(target_x, target_y, duration_ms, delay_after=0.0):
    ...
    mid_x = (start_x + target_x) / 2 + random.randint(int(-distance_x / 7), int(distance_x / 7))
    mid_y = (start_y + target_y) / 2 + random.randint(int(-distance_y / 7), int(distance_y / 7))

    # Adding horizontal deviation if the movement is mostly vertical
    if distance_x <= distance_y / 4:  # if the x distance is significantly less than the y distance
        mid_x += random.randint(int(-distance_y / 7), int(distance_y / 7))  # horizontal deviation
    ...
    for i in range(steps):
        ...
        current_x = (one_minus_t * one_minus_t * start_x + 2 * one_minus_t * progress * mid_x + progress * progress * target_x)
        current_y = (one_minus_t * one_minus_t * start_y + 2 * one_minus_t * progress * mid_y + progress * progress * target_y)
